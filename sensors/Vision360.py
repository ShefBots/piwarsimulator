...
from omnicam.protocol import (
    Mode,
    COMMUNICATION_PORT,
    REMOTE_ADDR,
    MODE_IDENTIFIER_INDICATOR,
)
...
class Vision360(Sensor):
    ...
    def __init__(self, brain_type, remote_connect=False):
        ...
        self._websocket_thread = threading.Thread(
            target=Vision360.handle_websocket_connection, args=(self, remote_connect)
        )
    ...
    def handle_websocket_connection(self, remote_connect):
        ...
        try:
            ...
            with sync_connect(f"ws://{server_addr}:{COMMUNICATION_PORT}") as websocket:
                ...
                print(
                    f'Transmitting mode "{self._brain_type}" command to the server...'
                )
            ...
            raise Vision360SocketError(
                "Connection refused by server. Check the server's status (and/or HOST LOCATION) and try again.",
                e,
            )
        except TimeoutError as e:
            raise Vision360SocketError(
                "Connection timed out. Check the server's status (and/or HOST LOCATION) and try again.",
                e,
            )
        except OSError as e:
            raise Vision360SocketError(
                f"Connection failed due to TCP connection failure. Likely a network or system issue.\n{e.strerror}",
                e,
            )
        except websockets.exceptions.InvalidHandshake:
            raise Vision360SocketError(
                "Connection failed due to an invalid handshake. This shouldn't happen. ...is there a corrupted network path?",
                e,
            )
